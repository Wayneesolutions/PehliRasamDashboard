          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {previews.map((src, idx) => (
              <div key={idx} className="relative rounded-md overflow-hidden shadow border bg-white group">
                <img src={src} alt={`preview-${idx}`} className="w-full h-48 object-cover" />
                <button onClick={() => handleRemovePreview(idx)} className="absolute top-2 right-2 bg-red-600 !text-white rounded-full w-8 h-8 flex items-center justify-center opacity-0 group-hover:opacity-100 transition" title="Remove image">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {customer.photos.map((photo) => (
              <div key={photo._id} className="relative rounded overflow-hidden shadow hover:shadow-lg transition-shadow">
                <img src={resolvePhotoUrl(photo.url)} alt="Customer Uploaded" className="w-full h-48 object-cover" />
